...
3. **Environment variables** (Vercel → Project → Settings → Environment Variables, Production):
   ...
   | `CRON_SECRET` | random; Vercel Cron sends it as `Authorization: Bearer …` — the only way the route accepts it, never in the URL |
...
**There is no subscription.** A boss pays **$2 an introduction and nothing else** — no monthly fee, no free trial, nothing to start, cancel or resume, and nothing that can lapse. The fortnight a boss is billed on opens the moment the account becomes a boss (`actions/auth.ts`) and runs **exactly 14 × 24 hours** from there (`FORTNIGHT_DAYS`, `addDays` — instant arithmetic, so a fortnight straddling a Sydney daylight-saving change is still a fortnight and the boundary never walks an hour). Each boss sits on **their own** 14-day grid anchored to their own signup day, not a shared calendar date, which is what keeps the cron's `LIMIT 500` spread across the fortnight instead of the whole book landing on one morning.

**Closing a fortnight** (`closePeriod`, `lib/invoicing.ts`) is one idempotent step in one transaction: lock the boss's row, do nothing if the fortnight isn't over, write **one match line per introduction billed in it**, and move the fortnight on by another 14 days. An invoice carries **only match lines** — there is no other kind. **No invoice is written for nothing**, which under $2-a-match is most fortnights, so `closePeriod` answers `{ closed, invoiced }` with those two facts kept apart: a caller that read "no invoice" as "nothing happened" would close the same fortnight twice. Running it twice writes nothing the second time — the row lock, `invoices (boss_id, period_start)` being unique, and a billed introduction being stamped with its line are three separate reasons. `matchLines` has **no lower bound**, so an introduction that missed its own invoice is picked up by the next one rather than quietly forgotten. Invoice numbers are `OS-2026-000123`, unique and sequential within the Sydney year (`invoice_counters`). Dates are Sydney dates; invoices are due **`PAYMENT_TERMS_DAYS` (7) days** after they are issued — a separate constant from the fortnight, and deliberately shorter than it.

**Nothing is gated. Nothing switches off, ever.** Posting shifts, matching, Workers, approving hours, the pay run (`/boss/pay`), the record and the CSV export (`/boss/pay/export`) are all open to every boss, whatever they owe and however overdue it is. There is no status to be in and no upsell to render. Approving in particular must stay free, because approving is what creates a billable match, and charging for it would mean charging a boss to be charged.

**Cron.** `/api/cron/expand` (every 20 min) calls `closeBillingPeriods()` after matching, in its own try/catch so a billing fault can never stop a shift being filled, and reports `billing: { closed, invoiced }` — counts only, never an id, and `closed` is always the larger, because a fortnight with no introductions in it closes without an invoice. While QPay is configured it also warms the rate (`warmAudToMnt`): it asks the Bank of Mongolia only when there is no official rate cached in the last 6 hours (a fresh fallback rate doesn't stop it trying again), and reports `fx: { source, as_of }` (nulls when there is no rate or QPay is off).
...
**Screens.** `/boss/money` puts the pay run and the open invoices on **one** fortnight window (`lib/fortnight.ts`, the boss's own billing fortnight — not a calendar one, or half of one invoice would sit beside half of another). `/boss/billing` is the invoice list plus what this fortnight is running at, and `/boss/billing/[number]` is the tax-invoice layout followed by the QPay block; both are linked from `/boss/me` and from `/boss/money`. The **$2 story is told once**, on `/boss/approve`, beside the hours that cause the charge.
...
**Migration 021** re-anchors every boss onto a 14-day grid starting at their own signup day (`users.created_at`), so the fortnights are spread across the calendar rather than bunched on one date, and indexes `bosses(period_ends_at)` for the cron's unfiltered sweep. It is **additive**: it drops nothing. `bosses.subscription_status` keeps its `NOT NULL DEFAULT 'trialing'` from migration 010 — so a boss `INSERT` that no longer mentions it still succeeds — and `trial_ends_at`, `subscription_cancelled_at`, `bosses_trial_idx` and the old partial `bosses_period_idx` are all left in place, **written by nothing and read by nothing**. A later deploy can drop them once nothing has written them for a while; a billing rollback must not have to drag a schema teardown with it.

**Migration 010** adds the columns and tables and backfills: existing non-direct booking pairs become introductions already marked billed, with no invoice line, so no one is charged for their own history. Its trial and monthly-period columns are the ones migration 021 leaves behind.

## The rules (`/terms`) and privacy (`/privacy`)

`/terms` is the plain-English agreement, in ten short sections: what OnSite is (and that **OnSite is not the employer**), that the boss is the employer for every shift they post, what workers agree to, what bosses agree to, the fees, what happens when the two sides disagree about hours, what blocking / leaving a crew / deleting an account each do, the demo warning (only on a demo deployment), changes and **the law of New South Wales**, and the contact. Every figure on it is read **when the page is requested** from the same constants the billing code charges from — `AWARD_CASUAL_FLOOR` (`lib/award.ts`), `MATCH_FEE_CENTS`, `FORTNIGHT_DAYS`, `paymentTermsDays()`, `GST_REGISTERED` (`lib/subscription.ts`) — so a price can never be stale on the one screen that promises it, and no email address or ABN is ever typed into the page (`tests/unit/launch.test.ts` checks both, the same way it checks `/privacy`).

It says only what the code does. Two places where that is worth knowing: **an overdue invoice switches nothing off** — nothing in the app is switched off for not paying, because there is nothing to switch off and no gate left in the code to do it — and **only bosses can block**, there is no worker-side block.
...
- Visa type is display-only. A worker never needs an ABN — every shift is employment, not a contract. (A boss's own ABN is optional, and only ever appears on their own invoices; it is checked against the ATO's own sum at onboarding and in Settings, so a mistyped one is handed back rather than printed.)
