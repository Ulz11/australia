...
export function CrewsSection({ crews }: { crews: CrewRow[] }) {
  return (
    <div className="card space-y-3">
      <div className="text-lg font-bold flex items-center gap-2"><HardHat size={20} strokeWidth={2.25} aria-hidden className="shrink-0" />Crews you&apos;re in</div>
      {crews.length === 0
        ? <p className="text-steel">Nobody has you on their crew list yet. A boss you work for can add you, and then book you without going through matching.</p>
        : (
            ...
            {crews.map((c) => (
              <li key={c.boss_id} className="py-3 flex items-center gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-sm text-steel">They can book you directly.</div>
                </div>
                <ConfirmButton action={leaveCrew.bind(null, c.boss_id)} className="btn-ghost btn-sm shrink-0"
                  title={`Leave ${c.company}'s crew?`}
                  details={["They can't book you directly any more.", "Their shifts still reach you through matching.", "They aren't told."]}
                  confirmLabel="Yes, leave" cancelLabel="Stay">Leave</ConfirmButton>
              </li>
            ))}
          </ul>
        )}
    ...
    <div className="say-dark">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="say-title">You&apos;re on {crew.company}&apos;s crew list.</div>
          <div className="say-sub">They can book you directly.</div>
        </div>
      </div>
      <div className="mt-3">
        <ConfirmButton action={leaveCrew.bind(null, crew.boss_id)} className="btn bg-white text-ink w-full"
          title={`Leave ${crew.company}'s crew?`}
          details={["They can't book you directly any more.", "Their shifts still reach you through matching.", "They aren't told."]}
          confirmLabel="Yes, leave" cancelLabel="Stay">Leave</ConfirmButton>
      </div>
    </div>
