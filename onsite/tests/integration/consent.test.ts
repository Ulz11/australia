/**
 * Consent to the privacy notice and the rules at onboarding, enforced by the server action — not just the checkbox.
...
describe.skipIf(!process.env.DATABASE_URL)("privacy consent at onboarding", () => {
  const ids: string[] = [];
  const me = async (i: number) => (await sql`SELECT role, name, privacy_accepted_at, privacy_version, terms_accepted_at, terms_version FROM users WHERE id = ${ids[i]}`)[0];
  ...
  it("without the box ticked nothing is set up, and they're sent back to onboarding with a reason", async () => {
    ...
    for (const form of [
      ...
      expect(await me(0)).toEqual({ role: null, name: null, privacy_accepted_at: null, privacy_version: null, terms_accepted_at: null, terms_version: null });
    }
  ...
  it("with the box ticked, a worker is set up and both consents are stamped with the version on screen", async () => {
    vi.stubEnv("TEST_USER_ID", ids[1]);
    expect(await onboard(fd({ role: "worker", name: "Consent Worker", privacy: "yes" }))).toBe("/worker");
    const u = await me(1);
    expect(u).toMatchObject({ role: "worker", name: "Consent Worker", privacy_version: PRIVACY_VERSION, terms_version: TERMS_VERSION });
  ...
  it("seeded demo accounts count as having agreed, so the local demo keeps working", async () => {
    ...
    expect(demo.filter((u) => !u.terms_accepted_at || u.terms_version !== TERMS_VERSION)).toEqual([]);
  });
});
