    ...
    <div className="space-y-3">
      ...
      <div className="card p-3">
        <div className="flex items-center justify-between mb-2">
          <button className="btn-ghost btn-sm" aria-label={t("Month before")} onClick={() => moveMonth(-1)}><ChevronLeft size={22} strokeWidth={2.5} aria-hidden /></button>
        ...
        <div className="flex gap-4 text-sm text-steel mt-3 flex-wrap font-semibold">
          <L c="bg-go" t={t("Free")} /><L c="bg-go/20" t={t("Usually free")} /><L c="bg-site border border-line" t={t("Busy")} /><L c="bg-ink" t={t("Working")} /><span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-full bg-hv" />{t("Offer for you")}</span>
        </div>
      </div>

      <div className="card space-y-3">
        <div className="text-xl font-extrabold">{sel === today ? t("Today") : fmtDay(sel, locale)}</div>

        {selBook ? (
          <div className="say-dark">
            <div className="say-sub">{t("You're working")}</div>
            <div className="say-title">{selBook.site}</div>
            <div className="say-sub">{fmtTime(selBook.start_time)} · {t("{n} hours", { n: selBook.hours })} · <a href="/worker/shift" className="underline font-bold">{t("open My shift")}</a></div>
          </div>
        ) : (
          <div>
            <div className="text-base font-bold mb-1.5">{t("Can you work this day?")}</div>
            <div className="seg grid-cols-2">
              <button disabled={pending} onClick={() => flip("free")} className={`seg-item ${status === "free" || status === "usual" ? "bg-go text-white" : ""}`}>{t("Yes, I'm free")}</button>
              <button disabled={pending} onClick={() => flip("busy")} className={`seg-item ${status === "busy" ? "bg-slab text-white" : ""}`}>{t("No, busy")}</button>
            </div>
            <div className="text-sm text-steel mt-1">
              {status === "usual" ? t("{weekday} is in your usual week, so bosses nearby can send you shifts for this day.", { weekday: t(WEEKDAY[isoDow(sel)]) })
                : status === "free" ? t("Bosses nearby can send you shifts for this day.")
                : t("You won't be asked about this day.")}
            </div>
          </div>
        )}

        {selShifts.length === 0 ? (
          !selBook && <p className="text-steel">{status === "free" || status === "usual" ? t("Nothing near you for this day yet. Your phone will buzz when a boss posts.") : t("Nothing to show.")}</p>
        ) : (
          <div className="space-y-2">
            <div className="text-base font-bold">{t("Shifts near you this day")}</div>
...
export function ShiftCard({ s, onTake, pending }: { s: S; onTake: () => void; pending: boolean }) {
  ...
  return (
    <div className={`rounded-2xl border-2 p-4 ${s.notified ? "border-hv bg-hv-soft" : "border-line bg-white"}`}>
      {s.notified && <Flag tone="orange" icon={BellRing} className="mb-2">{t("Offered to you")}</Flag>}
      <div className="text-2xl font-extrabold num">${Math.round(s.rate * s.hours)} <span className="text-base font-semibold text-steel">{t("for the day")}</span></div>
      <div className="text-lg font-bold mt-1">{s.role} · {s.site}</div>
      <div className="text-steel">{t("{time} start", { time: fmtTime(s.start_time) })} · {t("{n} hours", { n: s.hours })} · {t("${rate} an hour", { rate: s.rate.toFixed(2) })}</div>
      <div className="text-steel">{t("{km} from home", { km: km(s.dist_m) })} · {s.boss}{left > 1 ? ` · ${t("{n} spots", { n: left })}` : left === 1 && s.spots > 1 ? ` · ${t("last spot")}` : ""}</div>
      {need.length > 0 && (
        <div className={`mt-1 font-semibold flex items-start gap-1.5 ${s.tickets_ok ? "text-go" : "text-warn"}`}>
          {s.tickets_ok
            ? <><Check size={20} strokeWidth={2.5} aria-hidden className="shrink-0" />{t("You have the {cards} licence", { cards: need.map((k) => TICKETS[k] ?? k).join(", ") })}</>
            : <><X size={20} strokeWidth={2.5} aria-hidden className="shrink-0" />{t("Needs {cards} licence — you don't", { cards: need.map((k) => TICKETS[k] ?? k).join(", ") })}</>}
        </div>
      )}
      {s.ot_mode && (
        <div className="text-sm mt-1.5 rounded-lg bg-site px-2.5 py-1.5">
          <b>{t("Overtime agreed up front:")}</b> {otInWords({ ot_mode: s.ot_mode as never, ot_after_hours: s.ot_after_hours ?? 8, ot_multiplier: s.ot_multiplier ?? null }, s.rate)}
        </div>
      )}
      {(s.approve_h != null || s.pay_d != null) && <div className="text-sm text-steel mt-1">{t("This boss")} {s.approve_h != null ? t("approves hours in about {n} h", { n: s.approve_h }) : ""}{s.approve_h != null && s.pay_d != null ? ", " : ""}{s.pay_d != null ? t("pays in about {n} days", { n: s.pay_d }) : ""}.</div>}
      <button className="btn-primary mt-3" onClick={onTake} disabled={pending || !canTake}>{!s.tickets_ok ? t("Can't take — licence needed") : left <= 0 ? t("Filled") : t("Take it")}</button>
