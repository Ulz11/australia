...
export function ProfileCard({ p }: {
  ...
  async function pick(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setErr("Couldn't read that picture. Try another.");
    }
    setBusy(false);
  }
    ...
    <form action={async (fd) => { const r = await saveProfile(fd); if (r?.error) setErr(r.error); else { setSaved(true); setTimeout(() => setSaved(false), 2500); } }}
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1 space-y-2">
          ...
          {photo && (
            <button type="button" className="btn-ghost btn-sm w-full" onClick={() => start(async () => { setPhoto(null); await removePhoto(); })}>Remove</button>
          )}
          <input ref={file} type="file" accept="image/*" capture="user" className="hidden" onChange={pick} />
        </div>
      </div>
      <p className="text-sm text-steel -mt-2">A clear photo of your face, like a work ID. Bosses pick people they can recognise at the gate.</p>

      <Field label="Your name"><input name="name" defaultValue={p.name} className="input" required /></Field>

      <Field label="Years on the tools" hint="Roughly is fine.">
      ...
      <Field label="What work can you do?" hint="Tick everything you've actually done. This is what bosses search.">
        <div className="grid grid-cols-2 gap-2">
          {TRADES.map((t) => (
            <label key={t} className="cursor-pointer">
              <input type="checkbox" name="trades" value={t} defaultChecked={p.trades.includes(t)} className="peer sr-only" />
              <span className="chip justify-center w-full text-sm peer-checked:bg-ink peer-checked:text-white peer-checked:border-ink">{t}</span>
            </label>
          ))}
        </div>
      </Field>

      <Field label="Languages you speak" hint="Handy on a site where the foreman speaks your language.">
      ...
      <Field label="Anything else? (optional)" hint="One or two lines. Own tools, own car, early starts — that sort of thing.">
        <textarea name="about" defaultValue={p.about ?? ""} maxLength={400} rows={3}
          className="input py-3 min-h-[96px]" placeholder="10 years formwork. Own car and tools. Happy with early starts." />
      </Field>
