...
export function UsualWeek({ days, first }: { days: number[]; first: boolean }) {
  ...
  return (
    <div className="card space-y-3">
      <div className="text-lg font-bold">Your usual week</div>
      <p className="text-steel">Days you&apos;re usually free. Bosses see you on these days without you tapping each one. Tap any day below to change just that day.</p>
      <div className="grid grid-cols-4 gap-2">
        {DAYS.map(([iso, label]) => (
          <button key={iso} type="button" aria-pressed={on.includes(iso)} onClick={() => toggle(iso)}
            className={`chip justify-center w-full ${on.includes(iso) ? "bg-ink text-white border-ink" : ""}`}>{label}</button>
        ))}
      </div>
      ...
      <p className="text-sm text-steel">Stops after 14 days without opening the app, so nobody&apos;s shown someone who&apos;s moved on.</p>
    </div>
  );
}
